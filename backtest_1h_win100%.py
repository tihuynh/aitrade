...
def backtest_ai_strategy(model, scaler, data, initial_balance=5000):
    ...
    for i in range(LOOKBACK, len(data)):
        ...
        ai_confidence = prediction > current_price + atr * 1.5
        macd_bullish = macd_val > macd_sig and macd_val > data.iloc[i-1]["macd"]
        rsi_ok = rsi_val > 50
        price_above_ema = current_price >= ema_val
        price_above_sma = current_price > sma_val

        buy_condition = ai_confidence and macd_bullish and rsi_ok and price_above_ema and price_above_sma

        if position == 0 and buy_condition:
            print(f"[CHECK] {timestamp} | Dự đoán: {prediction:.2f} | Giá hiện tại: {current_price:.2f} | Chênh lệch: {prediction - current_price:.2f}")
            print("✅ Thỏa điều kiện mua thông minh")

            position = 1
            buy_price = current_price
            take_profit = round(buy_price + atr * 2.5, 2)
            stop_loss = round(buy_price - atr * 1.5, 2)
            trade_log.append(f"🟢 Mua tại {buy_price:.2f} ({timestamp}) | TP: {take_profit:.2f}, SL: {stop_loss:.2f}")

        elif position == 1:
            if current_price >= take_profit:
                balance *= 1 + (atr * 2.5 / buy_price)
