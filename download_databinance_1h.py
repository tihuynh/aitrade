...
print(f"Bắt đầu tải {total_candles} nến BTCUSDT khung 1h từ Binance...")
...
for i in range(iterations):
    try:
        ...
        print(f"❌ Lỗi khi tải dữ liệu ở vòng {i + 1}: {e}")
        break
    if not data:
        print("❌ Không có dữ liệu, dừng quá trình.")
        break

    all_candles = data[::-1] + all_candles  # prepend thay vì append
...
# Lưu file
df.to_csv("data/backtest/binance_btc_1h.csv", index=False)
print("✅ Đã lưu file binance_btc_1h.csv thành công!")
